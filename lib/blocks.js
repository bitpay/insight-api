...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  return {
    ...
    poolInfo: this.getPoolInfo(coinbaseScriptBuffer)
  };
};
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.db.getBlockInfoByTimestamp(lte, gte, function(err, blocksInfo) {
    if(err) {
      return common.handleErrors(err, res);
    }

    if(blocksInfo.length > limit) {
      more = true;
      blocksInfo = blocksInfo.slice(0, limit);
    }

    for (var i = 0; i < blocksInfo.length; i++) {
      if(moreTs > blocksInfo[i].timestamp) {
        moreTs = blocksInfo[i].timestamp;
      }
    }

    blocksInfo.sort(function(a, b) {
      return b.height - a.height;
    });

    var data = {
      blocks: blocksInfo.map(function(info) {
        return {
          height: info.height,
          size: info.bytes,
          hash: info.hash,
          time: info.timestamp,
          txlength: info.transactionCount,
          poolInfo: self.getPoolInfo(info.coinbaseScriptBuffer)
        };
      }),
      length: blocksInfo.length,
      pagination: {
        next: next,
        prev: prev,
        currentTs: lte - 1,
...
BlockController.prototype.getPoolInfo = function(coinbaseScriptBuffer) {
  for(var k in this.poolStrings) {
    if (coinbaseScriptBuffer.toString('utf-8').match(k)) {
